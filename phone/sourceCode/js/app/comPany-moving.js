...
$(window).ready(function(){
	...
	function problemData(){
		dataObj.isenterpriseProblem.Answer = $("#isenterpriseProblem").find(".currentProblem-list").find("input:checked").val();
		dataObj.additionalProblem.Answer = $("#additionalProblem").find(".currentProblem-list").find("input:checked").val();
		dataObj.isfragileProblem.Answer = $("#isfragileProblem").find(".currentProblem-list").find("input:checked").val();
		...
		dataObj.isenterpriseProblem.problem = $("#isenterpriseProblem").find(".currentTitle>p").text();
		dataObj.additionalProblem.problem = $("#additionalProblem").find(".currentTitle>p").text();
		dataObj.isfragileProblem.problem = $("#isfragileProblem").find(".currentTitle>p").text();
...
//		console.log(startTme,"####",endtTme);
		$.post('/8KM/8GongLiXiangMu/fun/newOrder.php',{ orderType : "公司搬家" , thisUser : problemUserId , orderData:jsonData , oldTime:startTme , newTime:endtTme},function(res){
				console.log(res);
			if(res==1){
				mui.toast("完成提交");
				setTimeout(function(){
